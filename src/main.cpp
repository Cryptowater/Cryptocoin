...
#if defined(NDEBUG)
# error "Cryptocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0d8c6bce14cdb5113cfdf7177138e0d1b7ec643b2daedb0e6616002fdf87cc21");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryptocoin: starting difficulty is 1 / 2^12
...
{
    // Cryptocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1050000); // Cryptocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.5 * 24 * 60 * 60; // Cryptocoin: 3.5 days
static const int64 nTargetSpacing = 2.0 * 60; // Cryptocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryptocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cryptocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xef788822c032990ece0c2126c56c1895065416ef2c8e3dac0349df57f4232edc");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "New face of the 50 pound note revealed 15/07/2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044d94098838731ad816c319c1b56e8e97d214509e0bb60297271f838065dce6a2121269a78c0374a1093ede54639ec25e907a752f8687553bfb67a8b9a9f75408") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1563189688;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085350111;

        if (fTestNet)
        {
            block.nTime    = 1563189650;
            block.nNonce   = 385396077;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf34f4b197a731312c150def630de0db5c1210858d56b2b9e023ac467f2a18740"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfu, 0xc5, 0xb3, 0xd9 }; // Cryptocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptocoinMiner(CWallet *pwallet)
{
    printf("CryptocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptocoin-miner");
    ...
    try { loop {
        ...
        printf("Running CryptocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptocoinMiner, pwallet));
}
